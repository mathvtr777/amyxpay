...
if ($status === 'PAID_OUT') {
    ...
    $sql = sprintf("SELECT * FROM solicitacoes WHERE idtransaction = '$externalReference'");
    ...
    $sql = sprintf("UPDATE solicitacoes SET status = 'PAID_OUT' WHERE idtransaction = '%s'", $externalReference);
    $conn->query($sql);
